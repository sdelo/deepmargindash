...
function getEventTypeInfo(type: AdminEvent['type']): { label: string; color: string; bgColor: string } {
  switch (type) {
    ...
    case 'pool_config':
      return { label: 'Pool Config Update', color: 'text-amber-400', bgColor: 'bg-amber-500/20' };
    case 'deepbook_pool':
          ...
          return (
            <div
              ...
              className={`
                ...
                hover:bg-white/10
                ${isFirst ? 'border-amber-400/30' : 'border-white/10'}
              `}
            >
              {/* Timeline dot */}
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                {isFirst && (
                  <span className="text-xs font-semibold text-amber-400 bg-amber-400/10 px-2 py-1 rounded flex-shrink-0">
                    Latest
                  </span>
                )}
              </div>
