...
export function LiquidationDashboard() {
  ...
  const analytics = React.useMemo(() => {
    ...
    const averageLiquidationSize = totalLiquidations > 0 ? totalLiquidationVolume / totalLiquidations : 0;
  ...
  if (error) {
    return (
      <div className="card-surface p-6 rounded-3xl border border-red-500/20">
        <p className="text-red-400">Error loading liquidations: {error.message}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card-surface p-6 rounded-2xl border border-white/10">
          <div className="flex items-center justify-between mb-2">
            ...
            {isLoading && <div className="animate-pulse h-2 w-2 rounded-full bg-rose-400"></div>}
          </div>
      ...
      <div className="card-surface p-6 rounded-2xl border border-white/10">
        <h3 className="text-lg font-bold text-rose-300 mb-4">Liquidation History</h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  ...
                  <th className="text-right py-3 px-4 text-white/60 font-semibold">Pool Reward</th>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-white/80 text-sm mb-3">
                Total bad debt incurred: <span className="font-bold text-rose-400">
                  {(analytics.totalBadDebt / 1e9).toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}
              ...
              <p className="text-white/60 text-xs">
                Bad debt occurs when collateral is insufficient to cover the liquidation amount plus rewards.
                This represents a loss for the margin pool.
              </p>
            </div>
          </div>
        </div>
